...
!> Unit tests
module testapp_coarray_fpp_tests
  use fortuno_coarray, only : context => coa_context, test => coa_pure_case_item, is_equal,&
      & test_list
  implicit none
...
contains
  ...
  subroutine test_success(ctx)
    ...
    CHECK(ctx, is_equal(buffer, 1))

  end subroutine test_success

end module testapp_coarray_fpp_tests


!> Test app driving Fortuno unit tests
program testapp
  use fortuno_coarray, only : execute_coa_cmd_app
  use testapp_coarray_fpp_tests, only : tests
  implicit none
