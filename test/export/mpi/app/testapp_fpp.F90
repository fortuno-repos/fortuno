...
!> Unit tests
module testapp_mpi_fpp_tests
  use mpi_f08, only : mpi_bcast, MPI_INTEGER
  use fortuno_mpi, only : global_comm, is_equal, test => mpi_case_item, test_list, this_rank
  implicit none
...
end module testapp_mpi_fpp_tests


!> Test app driving Fortuno unit tests
program testapp
  use fortuno_mpi, only : execute_mpi_cmd_app
  use testapp_mpi_fpp_tests, only : tests
  implicit none
